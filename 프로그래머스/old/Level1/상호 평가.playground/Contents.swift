...
func solution(_ scores:[[Int]]) -> String {
    ...
    for i in 0 ..< newScores.count {
        if newScores[i][i] == newScores[i].max()! || newScores[i][i] == newScores[i].min()! {
            if newScores[i].filter({ $0 == newScores[i][i] }).count == 1 {
                newScores[i][i] = -1
            }
        }
    }
    ...
    for i in 0 ..< newScores.count {
        avg.append(newScores[i].filter { $0 != -1 }.reduce(0, +) / newScores[i].filter { $0 != -1 }.count)
    }
    
    return avg.map(switching(_:)).joined()
}

func switching(_ num: Int) -> String {
    if num >= 90 {
        return "A"
